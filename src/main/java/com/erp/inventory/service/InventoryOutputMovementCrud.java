...
import com.erp.inventory.exception.inventory.OutputMovementDoesNotExistException;
import com.erp.inventory.exception.inventory.StatusAlreadyReviewedException;
import com.erp.inventory.repository.InventoryOutputMovementRepository;
...
@Service
public final class InventoryOutputMovementCrud {

    private final static Logger LOG = LoggerFactory.getLogger(InventoryOutputMovementCrud.class);

    private final InventoryOutputMovementRepository repository;
    private final AuthenticatedUserProvider userProvider;

    public InventoryOutputMovementCrud(InventoryOutputMovementRepository repository, AuthenticatedUserProvider userProvider) {
        this.repository = repository;
        this.userProvider = userProvider;
    }

    public InventoryOutputMovementEntity create(InventoryOutputMovementEntity entity) {
        try {
    ...
    public ResponsePaginationDTO<InventoryOutputMovementEntity> searchByPage(PaginationRequestDTO paginationDTO) {
        try {
            Pageable pageable = PageRequest.of(paginationDTO.page().number(), PaginationRules.FETCH_SIZE, Sort.by("createdAt").descending());

            Page<InventoryOutputMovementEntity> page = repository.findAll(pageable);
    ...
    public InventoryOutputMovementEntity update(InventoryOutputMovementEntity entity) {
        try {
            InventoryOutputMovementEntity entityFound = repository.findById(entity.getId()).orElseThrow(() -> new OutputMovementDoesNotExistException(HttpCode.conflict()));
            ...
            entityFound.setOutputReason(entity.getOutputReason());
    ...
    public InventoryOutputMovementEntity approve(InventoryOutputMovementEntity entity) {
        try {
            InventoryOutputMovementEntity entityFound = repository.findById(entity.getId()).orElseThrow(() -> new OutputMovementDoesNotExistException(HttpCode.conflict()));
    ...
    public InventoryOutputMovementEntity reject(InventoryOutputMovementEntity entity) {
        try {
            InventoryOutputMovementEntity entityFound = repository.findById(entity.getId()).orElseThrow(() -> new OutputMovementDoesNotExistException(HttpCode.conflict()));
