...
DROP TABLE IF EXISTS assignment_sections CASCADE;
...
CREATE TABLE checklist_assignments (
    ...
    operator_full_name VARCHAR(1000) NOT NULL,
    mechanic_user_id INT NOT NULL,
    mechanic_full_name VARCHAR(1000) NOT NULL,
...
CREATE TABLE assignment_items (
    ...
    CONSTRAINT fk_assignment_items_assignment_sections
        FOREIGN KEY (section_id)
...
CREATE TABLE assignment_responses (
    ...
    CONSTRAINT ck_assignment_responses_comment_when_not_ok
        CHECK (
            (status IS NULL)
            OR
            (status = 'OK')
    ...
    CONSTRAINT fk_assignment_responses_checklist_assignments
        FOREIGN KEY (assignment_id)
        REFERENCES checklist_assignments (id)
        ON DELETE CASCADE,

    CONSTRAINT fk_assignment_responses_assignment_items
        FOREIGN KEY (item_id)
