...
INSERT INTO permissions (module_id, code, name, description) VALUES ((SELECT id FROM app_modules WHERE code = 'REPORT_TEMPLATES'), 'REPORT_TEMPLATES_CONFIG_SECTIONS', 'Configurar Secciones', 'Permite gestionar las secciones dentro de una plantilla');
INSERT INTO permissions (module_id, code, name, description) VALUES ((SELECT id FROM app_modules WHERE code = 'REPORT_TEMPLATES'), 'REPORT_TEMPLATES_CONFIG_POINTS', 'Configurar Puntos de Inspección', 'Permite definir los puntos específicos a evaluar en la plantilla');
...
--username=jalexisrdv
--password=jalexisrdv
INSERT INTO users (id, first_name, middle_name, last_name, second_last_name, phone_number, username, password, enabled, token, token_expiration_date) VALUES(1, 'Jose', 'Alexis', 'Ramirez', 'del Valle', '555555555555', 'jalexisrdv', '$2a$10$Eg7bKj0VW61UjNxbtZtzRuRTw53l7Xwsd/PZuDcnN8MdtNwhF.vZO', false, NULL, NULL);
